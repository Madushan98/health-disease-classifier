import re
import pandas as pd
from sklearn import preprocessing
from sklearn.tree import DecisionTreeClassifier, _tree
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.model_selection import cross_val_score
from sklearn.svm import SVC
import csv
import warnings
import pickle

from new_chat import capture_diseases, getAllSymptomps



training = pd.read_csv('Data/new_Training.csv', encoding='utf-8')
testing = pd.read_csv('Data/new_Testing.csv', encoding='utf-8')
cols = training.columns
cols = cols[:-1]
x = training[cols]
y = training['prognosis']
y1 = y

reduced_data = training.groupby(training['prognosis']).max()

# mapping strings to numbers
le = preprocessing.LabelEncoder()
le.fit(y)
y = le.transform(y)

with open('decision_tree_model.pkl', 'rb') as f:
    clf = pickle.load(f)

severityDictionary = dict()
description_list = dict()
precautionDictionary = dict()

symptoms_dict = {}

def calc_condition(exp, days):
    sum = 0
    for item in exp:
        sum = sum+severityDictionary[item]
    if((sum*days)/(len(exp)+1) > 13):
        print("ඔබ වෛද්යවරයෙකුගෙන් උපදෙස් ලබා ගත යුතුය. ")
    else:
        print("එය එතරම් නරක නොවිය හැකි නමුත් ඔබ පූර්වාරක්ෂාව ගත යුතුය.")


def getDescription():
    global description_list
    with open('MasterData/new_Symptom_Description.csv', encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
            _description = {row[0]: row[1]}
            description_list.update(_description)

def getSeverityDict():
    global severityDictionary
    with open('MasterData/new_Symptom_severity.csv', encoding='utf-8') as csv_file:

        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        try:
            for row in csv_reader:
                _diction = {row[0]: int(row[1])}
                severityDictionary.update(_diction)
        except:
            pass

def getprecautionDict():
    global precautionDictionary
    with open('MasterData/new_Symptom_precaution.csv', encoding='utf-8') as csv_file:

        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
            _prec = {row[0]: [row[1], row[2], row[3], row[4]]}
            precautionDictionary.update(_prec)


def check_pattern(dis_list, inp):
    pred_list = []
    inp = inp.replace(' ', '_')
    patt = f"{inp}"
    regexp = re.compile(patt)
    pred_list = [item for item in dis_list if regexp.search(item)]
    if(len(pred_list) > 0):
        return 1, pred_list
    else:
        return 0, []
    

def sec_predict(symptoms_exp):
    df = pd.read_csv('Data/new_Training.csv')
    X = df.iloc[:, :-1]
    y = df['prognosis']
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.3, random_state=20)
    rf_clf = DecisionTreeClassifier()
    rf_clf.fit(X_train, y_train)
    # with open('decision_tree_model.pkl', 'rb') as f:
    #     rf_clf = pickle.load(f)
       
    symptoms_dict = {symptom: index for index, symptom in enumerate(X)}
    input_vector = np.zeros(len(symptoms_dict))
    for item in symptoms_exp:
        input_vector[[symptoms_dict[item]]] = 1

    return rf_clf.predict([input_vector])


def print_disease(node):
    node = node[0]
    val = node.nonzero()
    disease = le.inverse_transform(val[0])
    return list(map(lambda x: x.strip(), list(disease)))


def tree_to_code(tree, feature_names):
    tree_ = tree.tree_
    feature_name = [
        feature_names[i] if i != _tree.TREE_UNDEFINED else "සොයාගත නොහැක!"
        for i in tree_.feature
    ]

    chk_dis = ",".join(feature_names).split(",")
    symptoms_present = []

    while True:

        print("\nඔබ අත්විඳින රෝග ලක්ෂණයක් කියන්න පුලුවන්ද  \t\t", end="->")
        disease_input = input("")
        conf, cnf_dis = check_pattern(chk_dis, disease_input)
        print(conf, cnf_dis)
        if conf == 1:
            print("ඔබගේ රෝගලක්ෂණ  වලට අදාල යෙදුම්: ")
            for num, it in enumerate(cnf_dis):
                print(num, ")", it)
            if num != 0:
                print(f"ඔබ අදහස් කළ එක තෝරන්න (0 - {num}):  ", end="")
                conf_inp = int(input(""))
            else:
                conf_inp = 0

            disease_input = cnf_dis[conf_inp]
            break
            # print("Did you mean: ",cnf_dis,"?(yes/no) :",end="")
            # conf_inp = input("")
            # if(conf_inp=="yes"):
            #     break
        else:
            print("සමාවෙන්න ඔබ කියන රෝග ලක්ෂණ ගැන මම දන්නේ නෑ ")

    while True:
        try:
            num_days = int(input("හරි දවස් කීයෙන්ද? :"))
            break
        except:
            print("වලංගු රෝග ලක්ෂණයක් ඇතුළත් කරන්න.")
    def recurse(node, depth):
        indent = "  " * depth
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            name = feature_name[node]
            threshold = tree_.threshold[node]
            if name == disease_input:
                val = 1
            else:
                val = 0
            if val <= threshold:
                recurse(tree_.children_left[node], depth + 1)
            else:
                symptoms_present.append(name)
                recurse(tree_.children_right[node], depth + 1)
        else:
            present_disease = print_disease(tree_.value[node])
            # print( "You may have " +  present_disease )
            red_cols = reduced_data.columns
            symptoms_given = red_cols[reduced_data.loc[present_disease].values[0].nonzero(
            )]
            # dis_list=list(symptoms_present)
            # if len(dis_list)!=0:
            #     print("symptoms present  " + str(list(symptoms_present)))
            # print("symptoms given "  +  str(list(symptoms_given)) )
            print("මෙම රෝග ලක්ෂණ ඔබට තිබෙනවද  ")
            symptoms_exp = []
            for syms in list(symptoms_given):
                inp = ""
                print(syms, "? : ", end='')
                while True:
                    inp = input("")
                    if(inp == "ඔව්" or inp == "නැත"):
                        break
                    else:
                        print("නිසි පිළිතුරු සපයන්න i.e. (yes/no) : ", end="")
                if(inp == "ඔව්"):
                    symptoms_exp.append(syms)

            second_prediction = sec_predict(symptoms_exp)
            # print(second_prediction)
            calc_condition(symptoms_exp, num_days)
            if(present_disease[0] == second_prediction[0]):
                print("You may have ", present_disease[0])
                print(description_list[present_disease[0]])

                # readn(f"You may have {present_disease[0]}")
                # readn(f"{description_list[present_disease[0]]}")

            else:
                print("You may have ",
                      present_disease[0], "or ", second_prediction[0])
                print(description_list[present_disease[0]])
                print(description_list[second_prediction[0]])

            # print(description_list[present_disease[0]])
            precution_list = precautionDictionary[present_disease[0]]
            print("Take following measures : ")
            for i, j in enumerate(precution_list):
                print(i+1, ")", j)

            # confidence_level = (1.0*len(symptoms_present))/len(symptoms_given)
            # print("confidence level is " + str(confidence_level))

    recurse(0, 1)

disease_list = getAllSymptomps()
    
def tree_to_code_with_list(inp,tree, feature_names):
    tree_ = tree.tree_
    feature_name = [
        feature_names[i] if i != _tree.TREE_UNDEFINED else "සොයාගත නොහැක!"
        for i in tree_.feature
    ]

    chk_dis = ",".join(feature_names).split(",")
    symptoms_present = []
    
    disease_input = inp
    diseases = capture_diseases(inp, disease_list)
    print(diseases)
    if len(diseases) == 0:
        return "ඔබ අත්විඳින රෝග ලක්ෂණ පැහැදිලි නැහැ. තව රෝග ලක්ෂණ කියන්න පුලුවන්ද"
    disease_input = diseases[0]
    def recurse(node, depth):
        indent = "  " * depth
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            name = feature_name[node]
            threshold = tree_.threshold[node]
            if name == disease_input:
                val = 1
            else:
                val = 0
            if val <= threshold:
                output = recurse(tree_.children_left[node], depth + 1)
                return output
            else:
                symptoms_present.append(name)
                output = recurse(tree_.children_right[node], depth + 1)
                return output
        else:
            present_disease = print_disease(tree_.value[node])
            # print( "You may have " +  present_disease )
            red_cols = reduced_data.columns
            symptoms_given = red_cols[reduced_data.loc[present_disease].values[0].nonzero(
            )]

            # check if symptoms_given and diseases are same and add to symptoms_exp
            symptoms_exp = [element for element in list(symptoms_given) if element in diseases]
            if len(symptoms_exp) == 0:
                return_sentence = "ඔබට" + present_disease[0] + "තියෙන්න පුළුවන් "
            
            second_prediction = sec_predict(symptoms_exp)
            return_sentence = ""
            if(present_disease[0] == second_prediction[0]):
                
                return_sentence += "ඔබට" + present_disease[0] + "තියෙන්න පුළුවන් "
            else:
                return_sentence += "ඔබට" + present_disease[0] + " හෝ " + second_prediction[0] + " තියෙන්න පුළුවන් "
            try:
                precution_list = precautionDictionary[present_disease[0]]
            except Exception:
                return return_sentence
            return_sentence += "\nමේ සදහා "
            for i, j in enumerate(precution_list):
                return_sentence += j + " "
            return_sentence += "පියවර ගන්න."
            return str(return_sentence)
    output = recurse(0, 1)
    return output

getSeverityDict()
getDescription()
getprecautionDict()
print("\nඔබ අත්විඳින රෝග ලක්ෂණ  \t\t", end="->")
disease_input = input("")
output = tree_to_code_with_list(disease_input,clf, cols)
print(output)



# symptoms_exp = [element for element in list(symptoms_given) if element in diseases]
#             second_prediction = sec_predict(symptoms_exp)
#             return_sentence = ""
#             if(present_disease[0] == second_prediction[0]):
#                 return_sentence += present_disease[0] + "ඔබට තියෙන්න පුළුවන් "
#             else:
#                 return_sentence += present_disease[0] + " හෝ " + second_prediction[0] + "ඔබට තියෙන්න පුළුවන් "
#             try:
#                 precution_list = precautionDictionary[present_disease[0]]
#             except Exception:
#                 return return_sentence
#             return_sentence += "/nමේ සදහා "
#             print("පහත පියවර ගන්න : ")
#             for i, j in enumerate(precution_list):
#                 return_sentence += j + " "
#             return_sentence += "පියවර ගන්න."